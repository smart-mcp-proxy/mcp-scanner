...
def test_load_tools_array_format(clean_tools_file):
    tools, server_name = load_tools_from_file(clean_tools_file)
    assert len(tools) == 3
...
def test_load_tools_object_format():
    data = {"server_name": "test-server", "tools": [{"name": "test", "description": "Test tool"}]}
    with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
        json.dump(data, f)
        f.flush()

        tools, server_name = load_tools_from_file(f.name)
        assert len(tools) == 1
